...
public class BshipProtocol {
	...
	public BshipProtocol(BshipThread w, BshipThread z)
	{
		users[0]=new Game(w);
		users[1]=new Game(z); 				
	}					
		...
		private void mapShips(String[][] c)
		{
			...
			for (i=0;i<5;i++)
			{
				...
				{			
					for (k=0;k<10;k++)
					{
						if (c[j][k].equals(Bship.getShips(i)))
						{
							if (g==0)
							{
								x=j;
