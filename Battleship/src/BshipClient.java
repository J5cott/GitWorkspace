...
public class BshipClient {   
	...
	public BshipClient () throws IOException {
	...
	{		
		...
		{			
			...
			{
				Bship.getPlayers(1).setUser(in.readLine());	
			}				
			System.out.println("I'm still in the loop man.");
		}
		if (!start.equals("getmove"))			
		{
			Bship.getPlayers(0).setMove(true);
			JOptionPane.showMessageDialog(null,Bship.getPlayers(0).getUser()
			+" move.","",JOptionPane.INFORMATION_MESSAGE);
		}
		//Bship.getPlayers(0).getTimer().start();
	}	
	...
	public void results()
	{
		...
		{
			Bship.getPlayers(1).setBboard(x,y,Color.blue);
			Bship.getPlayers(0).setShots();
		}
		else if (results.startsWith("hit"))
		{
			Bship.getPlayers(0).setHits();			
			results=results.substring(results.indexOf(" ")+1);			
			if (results.startsWith("shipsunk"))
			{
				Bship.getPlayers(1).setShipsLeft();
				results=results.substring(results.indexOf(" ")+1);
				...
				{					
					for (i=x;i<=endp;i++)
					{
						Bship.getPlayers(1).setBboard(i,y,Color.black);
					}
				}
				else
				{
					for (i=y;i<=endp;i++)
					{
						Bship.getPlayers(1).setBboard(x,i,Color.black);
					}					
				}
			}			
			else
			{			
				Bship.getPlayers(1).setBboard(x,y,Color.orange);
			}
			Bship.getPlayers(0).setShots();			
		}
		else if (results.startsWith("theirshot"))
		{
			...
			x=Integer.parseInt(results.substring(0,1));
			y=Integer.parseInt(results.substring(2,3));
			if (!Bship.getPlayers(0).getWhatShip(x,y).equals(" "))
			{
				Bship.getPlayers(1).setHits();
				if (!Bship.getPlayers(0).isSunk(x,y
					,Bship.getPlayers(0).getWhatShip(x,y)))
					Bship.getPlayers(0).setBboard(x,y,Color.orange);
			}
			else
			  Bship.getPlayers(0).setBboard(x,y,Color.blue);
			Bship.getPlayers(1).setShots();				
		}
		...
		try
		{
		  ...
		  {
		  		Bship.setGameOver(true);
				if (Bship.getPlayers(0).getShipsLeft()!=0)
	...
	public void fireShot(int a, int b)
	{
		...
		out.println(x);
		out.println(y);
		System.out.println("shot fired:  "+Bship.getCletters(x+1)+","+Bship.getCnumbers(y+1));		
	}
	...
	{		
		out.println(Bship.getPlayers(Bship.getYou()).getUser());			
		for (i=0;i<10;i++)
			for (j=0;j<10;j++)
				out.println(Bship.getPlayers(Bship.getYou()).getWhatShip(i,j));		
	}
}
