...
public class Player
{
	...
	{			
		...
		if
		((user.equals("Computer"))||(user.equals("CPU1"))||(user.equals("CPU2"))||(Bship.isAutoSet())||(Bship.isLocal()))
			for (i=0;i<5;i++)
				boats[i]=new Ship(Bship.getShips(i),0,0,0,0);		
	...
	{
		if (Bship.getStats().isShowing())
		{	
			Bship.getStats().removeAll();
			Bship.getStats().setLayout(new GridLayout(6,3));					
			Bship.setData(new JLabel(""));
			Bship.getStats().add(Bship.getData());
			Bship.setData(new JLabel("Player 1",SwingConstants.CENTER));
			Bship.getStats().add(Bship.getData());
			Bship.setData(new JLabel("Player 2",SwingConstants.CENTER));
			Bship.getStats().add(Bship.getData());				
			Bship.setData(new JLabel("Names"));
			Bship.getStats().add(Bship.getData());
			if (Bship.getYou() == 0)			
				resetStats(Bship.getYou(),Bship.getEnemy());			
			else 
				resetStats(Bship.getEnemy(),Bship.getYou());			
			Bship.getStatistics().getContentPane().add(Bship.getStats());
			Bship.getStatistics().pack();
			Bship.getStatistics().repaint();
		}				
	}	
			
	public static void resetStats(int x,int y)
	{
		Bship.setData(new JLabel(Bship.getPlayers(x).getUser(),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Bship.getPlayers(y).getUser(),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel("Shots Taken"));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(x).getShots()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(y).getShots()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel("Hits"));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(x).getHits()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(y).getHits()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel("Shot Accuracy"));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Bship.getPlayers(x).getAcc(),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Bship.getPlayers(y).getAcc(),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel("Ships Left"));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(x).getShipsLeft()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());
		Bship.setData(new JLabel(Integer.toString(Bship.getPlayers(y).getShipsLeft()),SwingConstants.CENTER));
		Bship.getStats().add(Bship.getData());	
	}
	...
	public boolean isSunk(int x, int y)
	{
		...
		{
			Bship.getPlayers(Bship.getEnemy()).setShipsLeft();
			if
			((Bship.getPlayers(Bship.getYou()).getUser().equals("Computer"))||(Bship.getPlayers(Bship.getYou()).getUser().equals("CPU1"))||(Bship.getPlayers(Bship.getYou()).getUser().equals("CPU2")))
			{	
				for (int k=0;k<10;k++)
					for (int m=0;m<10;m++)
						if
				(this.boats[f].getName().equals(this.getWhatShip(k
							,m)))
						{
							Bship.getPlayers(Bship.getYou()).setMHS(k,m,2);
							this.setBboard(k,m,Color.black);
						}
				Bship.getPlayers(Bship.getYou()).setGo(2);				
				Bship.getPlayers(Bship.getYou()).setChit(false);
	...
	if (((isValid(x+1,y))&&((Bship.getPlayers(Bship.getYou()).getMHS(x+1,y)==3)||
							(Bship.getPlayers(Bship.getYou()).getMHS(x+1,y)==1)))||
			((isValid(x-1,y))&&((Bship.getPlayers(Bship.getYou()).getMHS(x-1,y)==3)||
							(Bship.getPlayers(Bship.getYou()).getMHS(x-1,y)==1))))		
			u=0;			
		else
			u=5;			
		...
		while (u<5)				
		{
			...
			{						
				...
				{							
					if ((isValid(i,y))&&((Bship.getPlayers(Bship.getYou()).getMHS(i,y)==3)||
						(Bship.getPlayers(Bship.getYou()).getMHS(i,y)==1)))
					{
						g+=1;
	...
	public boolean rshipsh(int x,int y)
	{
		...
		if (((isValid(x,y+1))&&((Bship.getPlayers(Bship.getYou()).getMHS(x,y+1)==3)||
							(Bship.getPlayers(Bship.getYou()).getMHS(x,y+1)==1)))||
			((isValid(x,y-1))&&((Bship.getPlayers(Bship.getYou()).getMHS(x,y-1)==3)||
							(Bship.getPlayers(Bship.getYou()).getMHS(x,y-1)==1))))		
			u=0;			
		else
			u=5;			
		...
		while (u<5)
		{
			...
			{	
				...
				{												
					if ((isValid(x,i))&&((Bship.getPlayers(Bship.getYou()).getMHS(x,i)==3)||
						(Bship.getPlayers(Bship.getYou()).getMHS(x,i)==1)))
					{
						g+=1;
						if (g==this.boats[u].getLength())
							break daloop;
	...
	public void scanArea(int x, int y)
	{
		if (this.getGo()==2)
		{
			if (this.isPlausible(x,y+1))
				{
					if (!Bship.getPlayers(Bship.getEnemy()).rshipsh(x,y+1))
					{	
						this.setMHS(x,y+1,0);
						this.scanArea(x,y);
					}								
				...
				{
					if (!Bship.getPlayers(Bship.getEnemy()).rshipsv(x+1,y))
					{	
						this.setMHS(x+1,y,0);
						this.scanArea(x,y);
					}							
	...
	{				
		this.setShots();
		if (Bship.getPlayers(Bship.getEnemy()).getHitOrMiss(x,y))
		{
			this.setHits();
			if (!Bship.getPlayers(Bship.getEnemy()).isSunk(x,y))
			{
				Bship.getPlayers(Bship.getEnemy()).setBboard(x,y,Color.orange);
		...
		{	
			Bship.getPlayers(Bship.getEnemy()).setBboard(x,y,Color.blue);
			if ((this.getUser().equals("Computer"))||(this.getUser().equals("CPU1"))||(this.getUser().equals("CPU2")))
			{	
				this.setMHS(x,y,0);
				this.setChit(false);
	...
	public void humanAttack(ActionEvent v)
	{
		...
		{				
			...
			{				
				for (j=0;j<10;j++)
				{					
					if (source==Bship.getPlayers(Bship.getEnemy()).getBboard(i,j))
					{								
						if ((Bship.getPlayers(Bship.getEnemy()).getBboard(i,j).getBackground()==Color.black)||
							(Bship.getPlayers(Bship.getEnemy()).getBboard(i,j).getBackground()==Color.orange)||
							(Bship.getPlayers(Bship.getEnemy()).getBboard(i,j).getBackground()==Color.blue))
						{
			...
			this.setMove(false);
			this.getTimer().stop();				
			if (Bship.getPlayers(Bship.getEnemy()).getShipsLeft()!=0)
			{						
				if (!Bship.getPlayers(Bship.getEnemy()).getUser().equals("Computer"))
					Bship.getPlayers(Bship.getEnemy()).setMove(true);					
				Bship.getPlayers(Bship.getEnemy()).getTimer().start();
				Bship.flipYou();
			}
			else
			{
				if (Bship.getPlayers(Bship.getEnemy()).getUser().equals("Computer"))//change once
					//menu options work
				{
	...
	{	
		...
		{				
			Bship.getPlayers(Bship.getYou()).compattack();			
		}
	}
	...
	{					
				...
				{							
					...
					{									
						...
						if (this.isSurrounded(r,c))							
							this.setMHS(r,c,0);							
						else if ((!Bship.getPlayers(Bship.getEnemy()).rshipsh(r,c))
							&&(!Bship.getPlayers(Bship.getEnemy()).rshipsv(r,c)))
						{							
							this.setMHS(r,c,0);
						}															
						else if ((Bship.getPlayers(Bship.getEnemy()).getShipsLeft()==1)
							&&(this.getLastShip()==0))
						{								
							for (int i=0;i<5;i++)
								if (Bship.getPlayers(Bship.getEnemy()).boats[i].getHitsLeft()!=0)
									this.setLastShip(Bship.getPlayers(Bship.getEnemy()).boats[i].getLength());								
						}							
						rows.clear();
						cols.clear();
					}
		...
		if (Bship.getPlayers(Bship.getEnemy()).getShipsLeft()>0)
		{
			if ((!Bship.getPlayers(Bship.getEnemy()).getUser().equals("CPU1"))
				&&(!this.getUser().equals("CPU2")))
				Bship.getPlayers(Bship.getEnemy()).setMove(true);	
			Bship.getPlayers(Bship.getEnemy()).getTimer().start();
			Bship.flipYou();
		...
		else
		{
			if (this.getUser().equals("Computer"))
			{
				...
				JOptionPane.INFORMATION_MESSAGE);
				if (Bship.getPlayers(Bship.getEnemy()).getUser().equals("Stupid"))
				...
				for (i=0;i<10;i++)
				{
					for (j=0;j<10;j++)
					{
						if ((!this.getWhatShip(i,j).equals(" "))
							...
							{
								this.setBboard(i,j,Bship.getColor());
							}
					}
				}									
			}
